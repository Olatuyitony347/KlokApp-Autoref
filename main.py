...
def test_proxy(proxy):
    try:
        test_url = 'https://api1-pp.klokapp.ai'
...
def get_working_proxy(proxy_list, used_proxies):
    max_retries = 5
    for _ in range(max_retries):
        try:
            ...
            if not available_proxies:
                print(f"{Fore.RED}No more proxies available!{Style.RESET_ALL}")
                return None
            ...
            if test_proxy(proxy):
                print(f"{Fore.GREEN}Found working proxy: {proxy}{Style.RESET_ALL}")
                return proxy
            else:
                print(f"{Fore.RED}Proxy failed, trying another...{Style.RESET_ALL}")
                used_proxies.append(proxy)
        except Exception as e:
            print(f"{Fore.RED}Error testing proxy: {str(e)}{Style.RESET_ALL}")
            continue
    
    print(f"{Fore.RED}Failed to find working proxy after {max_retries} attempts{Style.RESET_ALL}")
    return None
...
def send_chat(session, chat_headers, chat_data, proxy):
    max_attempts = 3
    attempt = 0
...
def perform_chats(session, headers, proxy):
    max_retries = 3
    retry_count = 0
    ...
    while retry_count < max_retries and successful_chats < num_chats:
        try:
            ...
            for i in range(successful_chats, num_chats):
                ...
                response = send_chat(session, chat_headers, chat_data, proxy)
...
def login(address, signature, message, proxy, ref_code):
    try:
...
def calculate_max_accounts(proxy_count, ref_count):
    ...
    return min(proxy_count, ref_count * 100)

def get_user_input(proxy_count, ref_count):
    max_accounts = calculate_max_accounts(proxy_count, ref_count)
    
    print(f"\n{Fore.CYAN}Available Resources:{Style.RESET_ALL}")
    print(f"Proxies: {proxy_count} (1 account per proxy)")
...
def main():
    ...
    # Initialize counters
    used_proxies = set()  # Track unique proxies used
    successful_accounts = 0
    
    num_accounts = get_user_input(len(proxies), len(refs))
    ...
    for i in range(num_accounts):
        ...
        if not proxy:
            print(f"{Fore.RED}Stopping: All proxies have been used!{Style.RESET_ALL}")
            break
            
        used_proxies.add(proxy)  # Add proxy to used set
        ip = get_ip_info(proxy)
        ...
        try:
            ...
            session = login(address, signature, message, proxy, ref_code)
            if not session:
            ...
            if perform_chats(session, session.headers, proxy):
    ...
    print(f"Total proxies used: {len(used_proxies)}")
